...
namespace JsonDatabase
{
    ...
    {
        TH_PROFILING_THREAD((m_name + "[" + std::to_string(data.threadIndex) + "]").c_str());
        class CrashDetector
        {
        public:
            CrashDetector(ThreadData& result) {
                m_signal = &result;
            }
            void normalExit()
            {
        ...
        if (data.func)
        {
            while (m_run.load()) {
                ...
                TH_PROFILING_BLOCK("Start thread work");
                (*func)();
                TH_PROFILING_END_BLOCK;
