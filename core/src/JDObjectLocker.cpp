...
namespace JsonDatabase
{
	...
	bool JDObjectLocker::lock(JDObjectInterface* obj) const
	{
		if(!obj)
		{
	...
	bool JDObjectLocker::unlock(JDObjectInterface* obj) const
	{
		if (!obj)
		{
			m_lastError = Error::objIsNullptr;
	...
	bool JDObjectLocker::isLocked(JDObjectInterface* obj) const
	{
		if (!obj)
		{
			m_lastError = Error::objIsNullptr;
	...
	JDObjectLocker::Error JDObjectLocker::getLastError() const
	{
		return m_lastError;
	}
	const std::string& JDObjectLocker::getLastErrorStr() const
	{
		switch (m_lastError)
		{
